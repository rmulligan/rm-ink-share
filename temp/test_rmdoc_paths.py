...
HCL_PATH = os.path.join(TEMP_DIR, f"path_test_{int(time.time())}.hcl")
RMDOC_PATH = os.path.join(TEMP_DIR, f"path_test_{int(time.time())}.rmdoc")

def create_hcl_with_paths(content, output_path):
    """Create an HCL file using vector paths for text."""
    try:
        # Configurations
        width = 1404
        height = 1872
        ...
        # Create HCL content
        hcl_content = f"""# Remarkable Paths Test
# Generated at {time.strftime("%Y-%m-%d %H:%M:%S")}

puts "size {width} {height}"

# Convert all text to paths so it can be edited on device
puts "line_width 1"
...
def convert_to_rmdoc(hcl_path, rmdoc_path):
    """Convert HCL to RMDOC format."""
    try:
        cmd = [DRAWJ2D_PATH, "-v", "-F", "hcl", "-T", "rmdoc", "-o", rmdoc_path, hcl_path]
        ...
        if os.path.exists(rmdoc_path):
            ...
            print(f"RMDOC file created: {rmdoc_path} (size: {file_size} bytes)")
            return rmdoc_path
        else:
            print(f"RMDOC file not created!")
            return None
    
    except Exception as e:
        print(f"Error converting to RMDOC: {e}")
        return None

def upload_to_remarkable(rmdoc_path, title):
    """Upload RMDOC to Remarkable Cloud."""
    try:
        # Create a safe title
        safe_title = "".join([c if c.isalnum() or c in " .-_" else "_" for c in title])
        safe_title = safe_title[:50]
        
        # Create a copy with the title as filename
        safe_path = os.path.join(TEMP_DIR, f"{safe_title}.rmdoc")
        import shutil
        shutil.copy2(rmdoc_path, safe_path)
...
# Main execution
print("Testing RMDOC with paths approach...")

# Create test content
test_content = {
    "title": "Test Paths Format",
    "url": "https://docs.mulligan.dev/test",
    "structured_content": [
        {"type": "paragraph", "content": "This is a test paragraph to verify if the vector path approach works."},
        {"type": "paragraph", "content": "If this shows up on your ReMarkable tablet and can be edited, the test was successful!"},
...
# Create HCL file
hcl_path = create_hcl_with_paths(test_content, HCL_PATH)

# Convert to RMDOC
if hcl_path:
    rmdoc_path = convert_to_rmdoc(hcl_path, RMDOC_PATH)
    
    # Upload to Remarkable
    if rmdoc_path:
        if upload_to_remarkable(rmdoc_path, test_content.get("title", "Test")):
            print("Success! RMDOC with paths uploaded to Remarkable.")
        else:
            print("Failed to upload RMDOC.")
    else:
        print("Failed to create RMDOC.")
else:
